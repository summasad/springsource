...
//2) @RestController : 리턴하는 값이 브라우저에 보여짐
//  -객체 리턴 기능 : 객체는 브라우저에 띄울 수 없음
//      ==> converter가 필요함( spring boot 가 자동으로 라이브러리를 가지고 있음)
//          객체 <==> json
...
public class TestController {
    ...
    public SampleDto2 getSample1() {
        return SampleDto2.builder().mno(11L).firstName("hong").lastName("dong").build();
    }
    ...
    public List<SampleDto2> getSample2() {
        ...
        LongStream.rangeClosed(1, 10).forEach(i -> {
            SampleDto2 sampleDto2 = SampleDto2.builder().mno(i).firstName("hong").lastName("dong" + i).build();
            list.add(sampleDto2);
        });
    ...
    public ResponseEntity<SampleDto2> check(double height, double weight) {
        SampleDto2 sampleDto2 = SampleDto2.builder().mno(11L).firstName(String.valueOf(height))
                .lastName(String.valueOf(weight)).build();
