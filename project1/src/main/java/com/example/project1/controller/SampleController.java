...
public class SampleController {
    ...
    // void or String (+ redirect)
    // void : templates 폴더 아래 경로로 인식
    // /basic => basic.html
    // /sample/ex2 => /sample/ex2.html
    // String : redirect 하는 경우이거나 template 파일명을 임의대로 지정하는 경우
    // templates 폴더 아래 return "파일명"
    // redirect: 주소명 - http://localhost:8080/주소명

    // 입력값 가져오기
    // 1) HttpServletRequest 사용 가능(입력값 가져오는 용도로는 잘 사용하지 않음)
    // 2) 매개변수 선언(변수명과 이름을 맞추는게 편함)
    // 3) DTO 를 사용(POST 메소드가 끝난 후 보여지는 페이지에서 DTO 사용 가능)
    // 클래스명 앞자리 소문자 (calcDto / loginDto)

    // 컨트롤러가 가지고 있는 값을 화면과 공유하기
    // redirect 로 움직이지 않았다면
    ...
    // 4) method(@ModelAttribute("uDto") UserDto uDto) : UserDto 공유하고 싶은데 이름을 다르게 공유
    ...
    // Model 과 RedirectAttribute 차이점 : 객체를 담을 수 있는지(model) 없는지(rttr)

    @GetMapping("/basic2")
    public String basic2(RedirectAttributes rttr) {
        log.info("basic2 컨트롤러 동작");
        rttr.addAttribute("age", 15); // 리다이렉트시 주소의 파라메터로 딸려 보내기
        rttr.addAttribute("name", "hong"); // 리다이렉트시 주소의 파라메터로 딸려 보내기
        rttr.addAttribute("addr", "seoul"); // 리다이렉트시 주소의 파라메터로 딸려 보내기

        // sendRedirect() : redirect:경로
        return "redirect:/ex1";
        // return "redirect:/ex1?age=15&name=hong";
        // //http://localhost:8080/ex1?age=15&name=hong
    }
    ...
    public String basic(RedirectAttributes rttr) {
        ...
        // http://localhost:8080/ex1;jsessionid=FD904BA8BFD28485BB0552AF81209F8D
        return "redirect:/ex1";
    }
