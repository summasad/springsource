...
public class FileChekTask {
    ...
    // 특정 스케줄에 메소드 호출(second / minute / hour /day of month/ month / day of week)
    @Scheduled(cron = "0 * * * * *")
    public void checkFile() {
        ...
        List<MovieImageDto> movieImageDtos = oldMovieImages.stream().map(movieImage -> {
            return MovieImageDto.builder()
                    .inum(movieImage.getInum()).uuid(movieImage.getUuid())
                    .imgName(movieImage.getImgName()).path(movieImage.getPath())
                    .build();
